...
bool Level::initialize(Json::Value& root) {
	...
	for (auto& sound : root["music"]["sounds"]) {
		...
		s.Type = static_cast<SoundInfo::BeatType>(sound.get("type", 0).asInt());
		s.Reset();
		// TODO: pitches
...
void Level::OnUpdate(sf::Time interval) {
	...
	for (auto& sound : m_sounds) {
		...
		if (sound.Type == SoundInfo::BeatType::Beat) {
			if (sound.Played) {
...
SoundInfo::SoundInfo() : Interval(1), Volume(1), Offset(0), Deviation(0), Sound(nullptr),
